...
data{
   int<lower=1> N1;//number of observations (years of RVC surveys)
  int<lower=1> N2;//number of observations (REEF surveys)
  real y1[N1]; //mean model estimate - RVC
  real sd_y1[N1]; //error in estimate - RVC
  ...
  int TT; // timespan
  int<lower=1,upper=TT> year_id1[N1]; // vector of year - RVC
  int<lower=1,upper=TT> year_id2[N2]; // vector of year - REEF
}
parameters {
  ordered[K-1] cut; //cutpoints
  vector[2] x0; //initial popn size - RVC/REEF

  //samples from RVC estimate
  vector[N1] est1;
...
model{
  ...
  for(i in 1:N1) est1[i] ~ normal(y1[i],sd_y1[i]);
  
  est1 ~ normal(x[year_id1,1],sd_r[1]);
